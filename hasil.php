    ...
    <table>
        ...
        <tbody>
            ...
            foreach ($saw->getAlternative() as $key) {
             echo "<tr id='data'>";
                echo "<td style='text-align:left;padding-left:15px;'>".$key['namaAlternatif']."</td>";
                $no=0;
                foreach ($saw->getNilaiMatriks($key['id_alternatif']) as $data) {
...
<div id="Normalisasi Matriks Keputusan">
    ...
    <table>
        ...
        <tbody>
            <?php
            //foreach data alternatif
            foreach ($saw->getAlternative() as $key) {
             echo "<tr id='data'>";
                echo "<td style='text-align:left;padding-left:15px;'>".$key['namaAlternatif']."</td>";
                $no=0;
                //foreach nilai alternatif
                foreach ($saw->getNilaiMatriks($key['id_alternatif']) as $data) {
                    //menghitung normalisasi
                    $hasil=$saw->Normalisasi($saw->getArrayNilai($data['id_kriteria']),$data['sifat'],$data['nilai']);
                    echo "<td>$hasil</td>";
                    $hitungbobot[$key['id_alternatif']][$no]=$hasil*$saw->getBobot($data['id_kriteria']);
                    $no++;
                }
                echo "</tr>";
            }
        ...
        <tbody>
            ...
            foreach ($saw->getAlternative() as $key) {
             echo "<tr id='data'>";
                echo "<td style='text-align:left;padding-left:15px;'>".$key['namaAlternatif']."</td>";
                $no=0;$hasil=0;
                foreach ($hitungbobot[$key['id_alternatif']] as $data) {
                    echo "<td>$data</td>";
                    //menjumlahkan
                    $hasil+=$data;
                }
                $saw->simpanHasil($key['id_alternatif'],$hasil);
                echo "<td>".$hasil."</td>";
                echo "</tr>";
            }
            ?>
